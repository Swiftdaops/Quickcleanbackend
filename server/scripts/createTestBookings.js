...
jest.setTimeout(30000); // 30 seconds timeout for DB connection
...
beforeAll(async () => {
  ...
  if (!uri) throw new Error('Please set MONGO_URI_TEST in your .env for tests');
...
afterAll(async () => {
  await mongoose.connection.dropDatabase(); // optional: clean test DB
  await mongoose.disconnect();
});

describe('Booking API', () => {
  const customerPhone = '08033005971';
